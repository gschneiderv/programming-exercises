...
#PK: its better to use the 'autoincrement' feature (it will appear in the 'Extra' column. So, I will ALTER the table, BUT atte!! 
...
# Nonexisting FK: you cant insert a number of person_id in favorite_table that doesnt exist in person table. 
# Column Value Violations: if a column is restricted to some values, you cant insert a differente that is not an option (ex: color_eyes)
# Invalid Date conversions: you cant write a string (a date) which is not in the format expected, ex: in birth_date will not match 'DEC-21-1980'
